...
Then $t$ induces a map $\overline{t} : F \> P \rightarrow G(H_{\Con}^n \> P)$ by replacing the constructors $c_{\nonrec}$ and $c_{\rec}$ by $c_{\nonrec}'$ and $c_{\rec}'$ respectively.
\end{lemma}
...
Another coherency we need is for the paths.
The paths use arguments from $B_i \> H$, and we want to prevent the presence of duplicates.
For this we define $C_3$ as the coequalizer of the following two arrows.
\[
\xymatrix
{
	B_i \> Q
	\ar@<-.5ex>[rrr]_-{H_{\Con} \> i \circ \overline{t_i}} \ar@<.5ex>[rrr]^-{\overline{t_i} \circ B_i(i \circ \inn_1)} 
	& & & 
	H_{\Con} \> P \ar[rrr]^-{\inC_2 \circ \inC_1}
	& & &
	C_2 
}
\]
Similarly, we define $C_4$ by replacing $\overline{t_i}$ by $\overline{r_i}$.
...
We define a map $q : B_i \> R \rightarrow \Sigma (x, y : P), (x = y) \times (x = y)$ sending $x : B_i \> R$ to the pair
\[
(\overline{t_i} \> x, \overline{r_i} \> x, \ap \> (i \circ \inn_1) \> (p_R \> x), p_Q \> (i_R \> x))
\]
Concluding the construction, we define $H_{\Approx}$ as $\pcoeq \> (B_i \> R) \> C_4 \> q$.
...
\begin{align*}
	F \> 0 &= H_{\nonrec},\\
	F \> 1 &= H_{\Approx} \> (F \> 0) \> \zero \> \zero \> \zero\frec \> \zero\frec \> \zero\frec \> \zero \frec \> \zero\frec\\
	F \> 2 &= H_{\Approx} \> (F \> 1) \> (F \> 0) \> \zero \> \inl \> \inr \> \zero\frec \> \zero\frec \> \zero\frec\\
	F(n+3) &= H_{\Approx} \> (F(n+2)) \> (F(n+1)) \> (F \> n) \> \inl \> \inr \> (f \> n) \> \overline{\glue} \> \overline{\glue} \qedhere
\end{align*}
where $\overline{\glue}$ is the map induced by the path $\glue$.
For the maps maps $f \> n : F \> n \rightarrow F(n + 1)$, note that we always have the following sequence of maps
\[
\xymatrix@1
{
	P \ar[r] & (I^1 \times B_i \> P) + P \ar[r] & H_{\Approx}.
}
\]
...
Now it computes overall due to the second added coherency which says that $\inC(\overline{t_i}(B_i(f \> n) \> x))$ and $\inC(H_{\Con}(f \> n)(\overline{t_i} \> x))$ are equal for all $x : B_i(F \> n)$.
...
The equality between the points is $\refl$ as they are definitionally equal.
...
Note that 
\[
h \> (n+1) \> (\inr(\inn_1 (y, x))) \equiv h \> n \> (\inn_1 (y, x)) \equiv \overline{q_{Y, i}}(y, h \> n \> x).
\]
\[
h \> (n+1) \> (I^1 \times B_i \> \inl \> (y, x)) \equiv h \> (n+1) \> (y, \inl \> x) \equiv \overline{q_{Y, i}}(y, h \> (n+1) \> (\inl \> x)) \equiv \overline{q_{Y, i}}(y, h \> n \> x).
\]
Hence, this coherency is fine as well.
Since they are definitionally equal, we can just take the path $\refl$.

Lastly, we need to check a coherency condition for the path $\glue$, and here the point is that all coherencies, including this one, are mapped to $\refl$.
This means that $\glue$ gets mapped mapped to $\refl$.
We thus take a look at the following corner
\[
\xymatrix
{
	(\two \times B_i \> (F \> n)) \times I^1
	\ar[d]_{(f \> n) \circ \overline{\glue}}
	\ar[rrrr]^-{\overline{\glue} \circ (\two \times B_i \> (f(n+1)))}
	& & & & F(n+2)
	\\
	F(n+2)
}
\]
The maps are induced by $\glue$.
\[
\ap \> (h \> (n+2)) \> (\glue(y, f \> n \> x)) = \refl?
\]
\begin{equation*}
\begin{split}
\ap & \> (h \> (n+2)) \> (\ap \> (f(n+1)) \> (\overline{\glue} \> (y, x)))
\\
&=
\ap \> (h \> (n+2) \circ f(n+1)) \> (\overline{\glue} \> (y, x))\\
&=
\ap \> (h \> (n+1)) \> (\overline{\glue} \> (y, x))\\
\end{split}
\end{equation*}
...
\section{Lemmata}
\label{sec:Lemmata}
In this section we prove the required lemmata to show that colimits over $\mathbb{N}$ commute with polynomials.
