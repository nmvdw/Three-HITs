...
\[
(\overline{t_i} \> x, \overline{r_i} \> x, \ap \> (j_Q \circ \inn_1) \> (p_R \> x), p_Q \> (j_R \> x))
\]
...
We need to check that $\inn_1(\inC \> x)$ and $H_{\Con} \> \inC \> x$ get mapped to the same element for $x : H_{\Con}(F \> n)$.
...
\begin{equation*}
\begin{split}
h \> (n+1) \> (H_{\Con} \> (\inC \circ \inn_1) \> (\inn_1 \> y)) 
&\equiv 
h \> (n+1) \> (\inn_1(\inC(\inn_1 \> y)))
\\
&\equiv
...
\end{split}
\end{equation*}
Here we use that $f \> x$ is defined to be $\inC(\inn_1 \> x)$.
Since they are definitionally equal, we can just use the path $\refl$.

For the other cases, we take a look at $c_{\rec}' \> y$.
\begin{equation*}
\begin{split}
h \> (n+1) \> (\inn_1(\inC \> (c_{\rec}' \> y))) 
&\equiv h \> (n+1) \> (\inC \> (c_{\rec}' \> y))\\
&\equiv h \> (n+1) \> (c_{\rec}' \> y)\\
&\equiv c_{Y, \rec} (h \> n \> y)
\end{split}
\end{equation*}
\begin{equation*}
\begin{split}
h \> (n+1) \> (H_{\Con} \> (\inC \circ \inn_1) \> (c_{\rec}' \> y)) 
&\equiv 
h \> (n+1) \> (c_{\rec}' \> (f \> n \> y))
\\
&\equiv
c_{Y, \rec} \> (h \> (n+1) \> (f \> n \> y)\\
&\equiv
c_{Y, \rec} \> (h \> (n+1) \> (\inC(\inn_1 \> y)).
\end{split}
\end{equation*}
...
For the second coherency we need to check that
